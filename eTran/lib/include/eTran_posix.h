...
static inline bool eTran_tcp_fail_if_header_copy(struct eTrantcp_connection *conn,
                                                 ...
                                                 size_t count)
{
    ...
    fprintf(stderr,
            "ERROR: TCP RX copy source is inside packet header\n"
...
static inline ssize_t eTran_tcp_rx_peek_count_zc(struct app_ctx_per_thread *tctx, struct eTrantcp_connection *conn, size_t count, void *buf)
{
    ...
    while (copy_offset < count) {
        ...
        if (eTran_tcp_fail_if_header_copy(conn, addr, pkt, expected_pos, rxmeta_pos(pkt),
